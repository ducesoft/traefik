...
func (t *Transport) Clone() *Transport {
	...
	if t.HTTP2 != nil {
		t2.HTTP2 = &http.HTTP2Config{}
		*t2.HTTP2 = *t.HTTP2
	}
	if t.Protocols != nil {
		t2.Protocols = &http.Protocols{}
		*t2.Protocols = *t.Protocols
	}
	if !t.tlsNextProtoWasNil {
		npm := maps.Clone(t.TLSNextProto)
		if npm == nil {
			npm = make(map[string]func(authority string, c *tls.Conn) http.RoundTripper)
		}
		t2.TLSNextProto = npm
	}
	return t2
...
func (t *Transport) protocols() http.Protocols {
	if t.Protocols != nil {
...
func (pc *persistConn) readResponse(rc requestAndChan, trace *httptrace.ClientTrace) (resp *http.Response, err error) {
	...
	for {
		resp, err = http.ReadResponse(pc.br, rc.treq.Request)
		if err != nil {
			return
		}
